...
**技术栈：** Python / LangGraph / LangChain / FastAPI / DeepSeek / Tool Calling / MCP / SSE / Qdrant
...
5. **会话抽象与工程验证：** 定义 ConversationMemory 接口并实现线程安全的 In-Memory 后端，通过 `user_id + conversation_id` 隔离会话；同时实现 Redis Adapter、TTL 与不可用时的自动降级，并提供 Dockerfile/Compose 部署模板。编写 13 项 pytest 测试覆盖图循环、工具调用、并发检索、会话隔离及 API 契约。
...
**技术栈：** Python / LangGraph / FastAPI / DeepSeek / Tool Calling / MCP / SSE / Qdrant
...
4. 抽象会话存储接口并以 In-Memory 后端完成运行验证，同时实现 Redis Adapter 与 Docker Compose 部署模板；13 项测试覆盖图循环、工具调用、并发检索、会话隔离与 API。
...
AgentHub 的核心是一个可扩展的 Agent 执行引擎。请求进入 FastAPI 后，LangGraph 根据当前消息状态调用 DeepSeek；模型可以直接回答，也可以生成结构化 Tool Call。ToolNode 执行计算、知识检索或网络搜索，并把结果作为 ToolMessage 写回图中，模型基于工具结果继续推理，直到生成最终答案。系统还通过 MCP 接入外部工具，通过 SSE 实时返回 Token；当前会话运行验证使用 In-Memory 后端，Redis Adapter 与 Docker Compose 作为待部署扩展保留在项目中。
